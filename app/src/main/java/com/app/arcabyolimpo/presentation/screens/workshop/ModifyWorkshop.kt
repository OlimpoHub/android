...
fun modifyWorkshopScreen(
    ...
    ArcaByOlimpoTheme(darkTheme = true, dynamicColor = false) {
        ...
        LaunchedEffect(formData.image) {
            if (formData.image.isNotEmpty() && selectedImageUri == null) {
                try {
                    selectedImageUri = Uri.parse(formData.image)
                } catch (e: Exception) {
                    Log.e("ModifyWorkshop", "Error parsing image URI: ${e.message}")
                }
            }
        }
                    ...
                    Row(
                        ...
                        if (showConfirmDialog) {
                            DecisionDialog(
                                ...
                                dialogText = "¿Deseas guardar los cambios en este taller? Asegúrate de que todos los datos sean correctos.",
